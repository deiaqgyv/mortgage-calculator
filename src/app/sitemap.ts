...
export default function sitemap(): MetadataRoute.Sitemap {
  ...
  const articles = ['amortization', 'extra-payments', 'affordability', 'methodology', 'editorial-policy', 'corrections-policy', 'legal-notice', 'privacy'];
  ...
  const content = articles.map((article) => ({ url: `${mortgageCalculatorUrl('en-us')}/${article}`, lastModified: new Date('2026-09-12') }));
